...
kyseisissä tutkimuksissa. Kirjallisuuskartoituksen tuloksena saatiin 
...
\begin{comment}
Tämä on vähän tautologinen eikä avaa asiaa lukijalle. kaikkein yksinkertaisin Monte Carlo on sitä, että lasketaan riippumattomia syöte-tulospareja, joissa syöte noudattaa annettua tlastollista jakaumaa ja tulosten jakaumasta lasketaan haluttuja tunnuslukuja lopputulokseksi. Monimutkaisempia MC algoritmeja tarvitaan tilanteissa, joissa riippumattomien näytteiden generointi syötejakaumasta on vaikeaa ja syötearvoja poimitaan tavalla, joka tekee niistä tilastollisesti keskenäsiriippuvia. Tällöin luonnollisesti tulosten analysoinnissa on huomioitava tämä riippuvuus. 
\end{comment}
...
\begin{comment}
Kun koetin perehtyä noihin työkaluihin, jäin miettimään, miten oikea termi tuo yleinen työkalu on, koska näissä mainituissa kyse oli enemmän yksittäisten tutkimusryhmien jakamista työkaluista. Yleisi käaupallisia tuotteita nämä eivät ole. Mutta ohjemistoja kannattaa avata enemmän myöhemmin.
\end{comment}
...
\begin{comment}
    muotoile niin, että tutkimuksen tavoite on tuottaa jotain (eikä saada!)

    Tämä oli se Kitchenham (Keelen yliopistosta). 
    Muuten näytät sanoittavan kaikkea saamisen kautta.

    Kitchenham raporttiin viittaus menee pieleen viittauksissa.
\end{comment}
...
\begin{comment}
Tästä taisin jo kommentoida aiemmin. Olisi hyvä avata vähän noita SD lukuja kahdesta eri aikaikkunasta. Niistä ja termin historiasta voi päätellä, että ennen 2000 on ollut aktiivisempaa kuin 2000-24, jos sama määrä viitteitä on tullut ennen 2000 vaikka käsiten on vuodelta 86 (vai mikä se nyt olikaan). Sinänsä on toki perusteltua tarkastella tuoreempaa tilannetta.
\end{comment}
...
\begin{comment}
On jo parempi, että taulukoita on vähemmän mutta edelleen sisällön kuvaaminen numeroilla (kategoria N) on viestinnällisesti huono. Näille on hyvä löytää sanalliset tunnisteet.
\end{comment}
...
\begin{comment}
Pystyykö heat mapin väräjä säätämään. Yleensä tummempi väri indikoi merkittävyyttä, joten nuo nollat vievät vusuaalisesti vähän harhaan. Saako värikartan käännettyä?
\end{comment}
...
\begin{tikzpicture}[scale=0.8]
  ...
  % row labels
  \foreach \a [count=\i] in {Agricultural and Biological,Chemical Sciences,Computer Sciences,Energy,Engineering,Environmental Science,Lifescience,Materials Science,Physics and Astronomy,Social and Psychology} {
    \node[minimum size=8mm] at (-3,-\i) {\a};
  }
...
\begin{comment}
Heittomerkkien ja Latexin kanssapitää olla tarkkana (tuuma merkki (shift 2) ja kaksi peräkkäistä apostrfia ovat eri asia (jälkimmäistä pitäisi käyttää) ja katsoa ette seuraavaa välilyöntiä syödä.
\end{comment}
...
\begin{comment}
Nyt työkaluissakin on ohjelmistoja, mikä osaltaan tekee listasta aika kirjavan. Lisäksi noita nimettyjä menetelmiä ei juuri avata, joten asioita tuntemattomalle, ne eivät kerro juuri mitään.
\end{comment}
...
kasvamista.
\begin{comment}
Ei pidä jättää sepkulatiiviseksi, 
koska sinulla on 25 vuoden aikasarja, 
josta näkee, miten avainsanan esiintyvyys on kehittynyt.
\end{comment}
...
\begin{comment}
Tässä taisi olla isoja eroja eri alojen kesken. Onko siitä mitään sanottavaa.
\end{comment}
...
\begin{comment}
Yhteiskuntatiede. Yksi perustavaa laatua oleva ero luonnontieteisiin voi olla se, että yhteiskunnallista interventiota ei voi kokeilla varovasti laboratoriossa, joten sen vaikutusten arviointia on ennakoitava sitäkin huolellisemmin. Tekniikassa kiinnostaa se, voiko proto toimia, koska lopulliset testit tehdään protolla ja viat tulevat sitä kautta esiin. Yhteiskunnalliset innovaatiot menevät suoraan tuotantoon (kuten globaalista politiikasta näemme päivittäin).
\end{comment}
...
\begin{comment}
Tätä kysyinkin tuolla taulukoiden kohdalla. Kannattaisiko nämä yhdistää taulukossakin, jos kyse on oleellisesti samanlaisesta lähestymistavasta ja ohjelmistotuotteesta.
\end{comment}
Garp3 
...
\end{itemize}
\begin{comment}
Tästä oli jo edellä. Ainakin hakululosten vuosittainen jakauma ja sen trendi pitäisi olla helposti tuotettavissa, jos data on laitettu hyvin tietokantaan. Toki aloijen sisäiset trendit tai ohjemistotrendit vaatisivat lisää analyysiä (jollainen on ihan tyypillistä kartoituskamaa).
\end{comment}
