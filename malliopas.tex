...
\begin{comment}
    Mitein kappaleen logiikkaa. tavallaan määrällinen tutkimus on suhteellisen selkeä. Laadullinen on sitten vähän sen komplementtia. 
    Lisäksi varsinakaan tässä työssä et tule rajaamana laadullista subjektiivisiin kokemuksiin jne. 
\end{comment}
\textbf{TODO: Määrällistä ja laadullista tutkimuksia pitää varmaan vielä avata enemmän.}
...
voidaan tehdä ennustuksia ja johtopäätöksiä puutteellisesta tai laadullisesta tiedosta \parencite{QualitativeReasoning1997}. 
Laadullinen päättely on käytössä usealla eri tieteenalalla, \textbf{(TODO: Viittaus kirjaan, korjaa mallipohjan mukaiseen viittaukseen)} kuten teköälyn \parencite{qualitativeReasoning2014} tai fysiikan aloilla \parencite{QualitativePhysics1988}.
...
\textbf{TODO: Avaa laadullista simulaatiota käsitteenä enemmän.}
...
\subsection{Hyödyt}
\begin{comment}
    Mietin, onko vähän liian suoraviivaista aloittaa hyödyistä kun määritelmä ja tavoitteet jäävät aika auki. tarvitaanko vahvemmin määrittelevä ja ominaisuuksia kuvaava alaluku. 
    
    Nyt jää käteen vain vähän epämääräinen toteamus soveltuvuudesta tilanteessa jossa riittävän tarkat tiedot puuttuvat.
\end{comment}
...
\end{comment}
\textbf{TODO: Miten QSIM-algoritmia ja muita algoritmeja voidaan käyttää.
Sama ohjelmistoille.}
...
\subsection{Ongelmat TODO: Sulautetaan aiempiin kappaleisiin mukaan.}
...
Virheellisenä toimintana seulonnasta ei pidetty tarkkaa kirjaa, 
miten jokainen syy vaikutti aineiston määrään.
...
luokiteltiin kyseiseen tieteenalaan. \textbf{TODO: Tarkempi tieteenala varmaankin turha, ehkä lisätään ne vain liitteeksi loppuun, niitä kuitenkin käytettiin Science Directin alojen korjauksessa.}
...
Tutkimuksen etsityt tulokset voivat sopia myös useampaan eri kategoriaan samaan aikaan. Löydetyt kategoriat voidaan jakaa 3 eri osioon. 
\textbf{TODO: Miten kategoriat nimetään, jos numerointi joudutaan poistamaan.}

\begin{comment}
    Tämä jäsennys kolmeen tuntuu hyvältä. 
    Kannattaisiko se tuoda peliin jo heti alusta (eli jäsentämään taulukkoa 3). 
    
    Nyt se numerojärjestys on aika satunnainen ja polkuriippuva (riippuu siitä, missä järjestyksessä aineistoa on kahlattu läpi ja tunnistettu näkökulmia). Jos tässä ei ole mitään selkeää logiikkaa, sitä ei ole numeroinnillakaan. 
    
    Ainut mieleen tuleva perustelu olisi, jos aineisto olisi käyty läpi ikä järjestyksessä ja isommat numerot viittaisivat myöhemmin agendalle tulleisiin kysymyksiin. (Sinänsä tuo ajallinen dynamiikka voisi olla ihan mielekäs tarkasteltava, jos muistiinpanot sen mahdollistavat). 
    
    Muuttuuko eri luokkien esiintyvyydet ajan kuluessa?
\end{comment}
...
\textbf{TODO: Taulukko uusiksi, sitten kun kategorioiden nimet on tarkennettu.}
...
Olisiko iso vaiva tehdä yksi taulukko jossa olisi toisena dimensiona laadullisen simuloinnin tavoite ja toisena päätieteenala. Saisiko tästä ehkä jopa heat mapin (missä soluissa on isot frekvenssit, missä pienemmät). 

Päätieteenalojen sisälle porautumista voisi miettiä myös sen edellä esitetyn kolmen pääluokan tasolla, mikä voisi keventää taulukoitavaa asiaa.
\end{comment}
...
\begin{comment}
Kategoria x on aika huonosti sisältöä kuvaava nimi. Kannattaa nimetä kuvaavammin. 

Samoin mietin, mistä tulee esitysjärjestys. Onko sille jokin perustelu? Science Directin luokkien numerointi?
\end{comment}
