...
\tiivistelma{%
Määrällinen simulaatio edellyttää huomattavaa määrää numeerista taustatietoa 
järjestelmän toiminnasta, kun taas laadullinen simulaatio mahdollistaa simuloinnin myös
silloin, kun järjestelmän toiminnan tarkasta tiedosta on puutteita. 
...
miten laadullisia simulaatioita pyrittiin hyödyntämään kerätyn aineiston tutkimuksissa.
Kirjallisuuskartoituksen tuloksena tuotettiin laaja kuvaus laadullisten simulaatioiden
käytöstä eri tieteenaloilla. Kartoituksessa tunnistettiin yhteensa 17 eri tieteenalaa ja 
...
simulaatioilla pyritään usein lisäämään ymmärrystä kohdejärjestelmän toiminnasta. 
Eniten simulaatioita on käytetty kemian tieteiden, tekniikan ja materiaalitieteiden 
aloilla. Laadullisten simulaatoiden toteutuksessa on käytetty niin yleisluontoisia
mallinnustyökaluja (kuten Matlab) kuin laadullisen simuloinnin erityisohjelmistoja 
(kuten QSIM).
}
\abstract{%
...
The main findings were that qualitative simulations are often used to increase understanding of how the target system works.
Simulations have been most widely used in the fields of chemical sciences, engineering and material sciences. 
Both generic modelling tools (such as Matlab) and specific qualitative simulation software (such as QSIM) have been used to implement qualitative simulations.
}
